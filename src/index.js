...
app.get('/api/chaves', async (req, res) => {
  try {
    const { rows } = await pool.query('SELECT * FROM proj_distrib ORDER BY idcodigo');
    res.json(rows);
  } catch (error) {
    res.status(500).json({ 
...
app.get('/api/chaves/:idcodigo', async (req, res) => {
  try {
    ...
    const { rows } = await pool.query('SELECT * FROM proj_distrib WHERE idcodigo = $1', [idcodigo]);
...
app.get('/api/chaves/codigo/:codigo', async (req, res) => {
  try {
    ...
    const { rows } = await pool.query('SELECT * FROM proj_distrib WHERE codigo = $1', [codigo]);
...
app.get('/api/chaves/email/:email', async (req, res) => {
  try {
    ...
    const { rows } = await pool.query('SELECT * FROM proj_distrib WHERE email = $1', [email]);
...
app.post('/api/chaves', async (req, res) => {
  try {
    ...
    const existingCode = await pool.query('SELECT * FROM proj_distrib WHERE codigo = $1', [codigo]);
    if (existingCode.rows.length > 0) {
      return res.status(409).json({ error: 'Código já existe' });
    }

    const { rows } = await pool.query(
      `INSERT INTO proj_distrib 
...
app.delete('/api/chaves/:idcodigo', async (req, res) => {
  try {
    const { idcodigo } = req.params;
    const { rows } = await pool.query('DELETE FROM proj_distrib WHERE idcodigo = $1 RETURNING *', [idcodigo]);
